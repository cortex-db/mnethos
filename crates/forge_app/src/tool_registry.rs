...
impl<S: Services + EnvironmentInfra<Config = forge_config::ForgeConfig>> ToolRegistry<S> {
    ...
    pub async fn tools_overview(&self) -> anyhow::Result<ToolsOverview> {
        ...
        // The memory tools (`remember` + `mem_search`) are offered only when
        // the `mnethos_memory` provider is configured (i.e. the user
        // has logged in and a credential is stored). The backend URL
        // and bearer token are then sourced from that provider, exactly
