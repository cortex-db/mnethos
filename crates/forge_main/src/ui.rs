...
impl<A: API + ConsoleWriter + 'static, F: Fn(ForgeConfig) -> A + Send + Sync> UI<A, F> {
    ...
    async fn handle_provider_login(
        ...
        // Non-LLM providers (e.g. the memory backend) have no models and must
        // not become the default chat provider, so skip model selection
        // / default activation. Configuring (the credential is already
        // stored) is the whole job for them.
    ...
    async fn handle_migrate_credentials(&mut self) -> Result<()> {
        ...
        if let Some(result) = result {
            self.writeln_title(
                TitleFormat::warning(
                    "Mnethos no longer reads API keys from environment variables.",
                )
