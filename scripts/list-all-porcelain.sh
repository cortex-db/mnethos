...
print_command "$MNETHOS_BIN list agent --porcelain"
start=$(date +%s%N)
$MNETHOS_BIN list agent --porcelain 2>&1 | head -20 || echo "No agents found"
print_runtime "$start"

# Provider list
print_section "PROVIDERS"
print_command "$MNETHOS_BIN list provider --porcelain"
start=$(date +%s%N)
$MNETHOS_BIN list provider --porcelain 2>&1 | head -20 || echo "No providers found"
print_runtime "$start"

# # Model list
# print_section "MODELS"
# print_command "$MNETHOS_BIN list model --porcelain"
# $MNETHOS_BIN list model --porcelain 2>&1 | head -20 || echo "No models found"

# Config list
print_section "CONFIG"
print_command "$MNETHOS_BIN list config --porcelain"
start=$(date +%s%N)
$MNETHOS_BIN list config --porcelain 2>&1 | head -20 || echo "No config found"
print_runtime "$start"

# MCP servers list
print_section "MCP SERVERS"
print_command "$MNETHOS_BIN list mcp --porcelain"
start=$(date +%s%N)
$MNETHOS_BIN list mcp --porcelain 2>&1 | head -20 || echo "No MCP servers found"
print_runtime "$start"

# Conversation list
print_section "CONVERSATIONS"
print_command "$MNETHOS_BIN list conversation --porcelain"
start=$(date +%s%N)
$MNETHOS_BIN list conversation --porcelain 2>&1 | head -20 || echo "No conversations found"
print_runtime "$start"

# Custom commands list
print_section "CUSTOM COMMANDS"
print_command "$MNETHOS_BIN list cmd --porcelain"
start=$(date +%s%N)
$MNETHOS_BIN list cmd --porcelain 2>&1 | head -20 || echo "No custom commands found"
print_runtime "$start"

# Skills list
print_section "SKILLS"
print_command "$MNETHOS_BIN list skill --porcelain"
start=$(date +%s%N)
$MNETHOS_BIN list skill --porcelain 2>&1 | head -20 || echo "No skills found"
print_runtime "$start"
